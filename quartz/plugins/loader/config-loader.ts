...
export async function loadQuartzConfig(
  ...
  // Site-specific transformers (see transformers/joshuaSite.ts): math must run
  // before plugin transformers (syntax highlighting), figures after OFM embeds.
  const builtinTransformers: unknown[] = [builtinPlugins.JoshuaMath()]
  const builtinTrailingTransformers: unknown[] = [builtinPlugins.JoshuaMarkup()]
