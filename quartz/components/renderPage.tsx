...
export function renderPage(
  ...
  const bodyClass =
    typeof frame.bodyClass === "function" ? frame.bodyClass(componentData) : frame.bodyClass
  const frameProps = {
    componentData,
    head: Head,
    header,
  ...
  const doc = frame.bare ? (
    // Bare frames own the whole <body>: no quartz-root/quartz-body wrappers.
    <html lang={lang} dir={direction}>
      {frame.head ? frame.head(componentData) : <Head {...componentData} />}
      <body class={bodyClass} data-slug={slug} data-basepath={basePath}>
        {frame.css && <style dangerouslySetInnerHTML={{ __html: frame.css }} />}
        {frame.render(frameProps)}
    ...
    <html lang={lang} dir={direction}>
      {frame.head ? frame.head(componentData) : <Head {...componentData} />}
      <body class={bodyClass} data-slug={slug} data-basepath={basePath}>
        <div id="mouse-trailer" />
        <script
          dangerouslySetInnerHTML={{
            __html: `
              (function () {
                var trailer = document.getElementById("mouse-trailer");
                if (!trailer) return;
        ...
        <div id="quartz-root" class="page" data-frame={frame.name}>
          <Body {...componentData}>{[frame.render(frameProps)]}</Body>
        </div>
      </body>
      {afterDOMReadyScripts}
    </html>
