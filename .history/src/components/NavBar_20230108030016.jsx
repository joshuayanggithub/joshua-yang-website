  ...
  return (
    <div class="md:flex bg-sasskyblue w-full justify-between items-center h-20 mx-auto px-4">
      <div onClick={toggleNav}>
        {nav ? <AiOutlineMenu> size={40}</AiOutlineMenu>: null}
      </div>

      <div class={!nav ? 'hidden' : "flex justify-between"}>
        <ul class="md:flex">
          {
            links.map((link) => 
              <li key={link.id}>
      ...
      <div class={!nav ? 'fixed left-0 top-0 w-[30%] h-full border-r border-r-gray-600 bg-sasnavyblue ease-in-out-duration-400' : "fixed hidden"}>
