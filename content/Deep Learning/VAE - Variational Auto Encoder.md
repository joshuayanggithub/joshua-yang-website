...
updated: "2026-03-12"
---
VAEs are a specific form of [[Autoencoder|autoencoders]]. However, they differ in that they are probabilistic models that encode latent variables of training data as a probability distribution, not a singular output. They are *generative* models, because they are probabilistic: we sample latent vectors from the distribution and decode it for output. The name variational comes from [[Variational Inference or Variational Bayes or Variational Bayes Inference|Variational Inference]]. 

# VAE Scenario
Our vanilla [[Autoencoder|autoencoders]] are purely for learning a useful latent representation - but we have no way of sampling useful $z$ latent vectors for proper generation. Hence VAEs solve the problem by conditioning our autoencoders to learn *continuous* and *structured* latent spaces, giving as an actual tool for *image generation* tasks!
...
![[Pasted image 20251129234007.png | The smile vector, trained on the CelebA dataset with VAEs ]]

# VAE Overview
![[Pasted image 20251228152436.png | Understanding the VAE Model | 600 ]]
...
3. our ***true* posterior** $p_{\theta}(z \mid x)$
	By Baye's Rule, $p_\theta(z\mid x) = \frac{p_\theta(x\mid z)\,p(z)}{p_\theta(x)}.$ This is intractable for the reasons above, and must be approximated, through network parameterized by $\phi$ instead!
4. our ***approximate/variational* posterior** $q_{\phi}(z \mid x)$ which is our **encoder**!
...
![[Deep Generative Modeling.png| 500]]
...
1. Convenient mathematic properties
...
2. Among all continuous distributions with a fixed mean and variance, the Gaussian has the largest **entropy**
	Intuitively, this means is that the latent prior is the most "uninformative" and the learned assumptions/patterns are maximally embedded within the encoder/decoder models that are actually used for image generation! 
...
\end{align}
$$
then we approximate using [[Variational Inference or Variational Bayes or Variational Bayes Inference|variational inference]], using our *approximate posterior* encoder $q_{\phi}(z \mid x)$ which outputs $\mu_{\phi}(x)$ and $\sigma_{\phi}^2(x)$ in order to describe a *gaussian distribution*. Note that our [[Variational Inference or Variational Bayes or Variational Bayes Inference|variational family]] need not be *gaussian*!, but we like for it to be because of easy parametrization and simplicity reasons.
...
![[Pasted image 20251228155437.png | 500]]
Now, fully our VAE can be shown as 
![[Pasted image 20251228155539.png | 700]]
# Decoder
...
2. **Regularization term** in order to push the encoder ([[Variational Inference or Variational Bayes or Variational Bayes Inference|approximate posterior]]) to match our gaussian prior. 
	We encourage the approximate posteriors to fit in a large gaussian, so that $\mu_\phi(x)$ is somewhat near the origin.  
	![[Pasted image 20251130012157.png | Illustration of ELBO term and regularization| 600]]
	With a gaussian prior, this expression has closed form: $$ D_{\mathrm{KL}}(q_\phi(z\mid x)\,\|\,p(z)) = \frac{1}{2}\sum_j \Big(\mu_j(x)^2 + \sigma_j(x)^2 - \log \sigma_j(x) -1 \Big) $$
...
$$\mathcal{L}(x; \theta, \phi) = - \frac{1}{N }\sum_{i=1}^N \log p_\theta(x_i) =  - \frac{1}{N} \sum_{i=1}^N \text{ELBO}(x_{i}) = - \frac{1}{N} \sum_{i=1}^N \log p_{\theta}(x \mid  z^{(i)}) - D_{KL}(q_{\phi}(z\mid  x) \parallel p(z)) $$

# Training VAEs
1. Feed a data point $x$ to encoder to predict $\mu_{\phi}(x)$ and $\sigma_{\phi}^2(x)$
2. Sample a latent variable $z$ from $q_{\phi}(z \mid x) = \mathcal{N}(z; \mu_{\phi}(x), \sigma_{\phi}^2(x))$ 
3. Feed $z$ through decoder to predict $\hat{x} = q_{\theta}(x \mid  z)$
4. Back propagate negative ELBO

![[VAE Training.jpg | 700]]
# Readings
